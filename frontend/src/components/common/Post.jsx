...
const Post = ({ post }) => {
	...
	const {mutate:deletePostMutation,isPending,isError,error}=useMutation({
		...
		onSuccess:()=>{
			...
			//Invaludate query to fetch the post
	...
	onSuccess:(updatedLikes)=>{
		...
		// queryClient.invalidateQueries({queryKey:["posts"]})
		//OPTIMAL SOLUTION (Update cache directly for the post)
	...
	const {mutate:commentPost,isPending:isCommenting}=useMutation({
		mutationFn:async()=>{
			const res=await fetch(`/api/posts/comment/${post._id}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:comment})})
			const data=await res.json()
			if(!res.json){
				return new Error(data.error || "Something went wrong")
			}
		...
		onSuccess:()=>{
			toast.success("Added comment on post")
			setComment("")
			queryClient.invalidateQueries({queryKey:["posts"]});
		},
		onError:(error)=>{
